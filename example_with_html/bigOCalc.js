...
function getBiggestBigOString(bigObjs) {
    ...
    var numberNsList = [];
    var maxNs = 0;
    //loop to calculate the number of N-s in each string and 
...
function semiColonSplit(forLine, level) {
    ...
    try {
        var varInstantiation = forLineLogic[0];//iterator declaration
        var range = forLineLogic[1];//second condition
        var operation = forLineLogic[2].split(")")[0];//rule for changing the iterator
...
function getBigONotation(forStatements) {
    //stores each for loop rate of growth and it's level
    var val_results = [];
    ...
    var bigORes;
    var bigOString = "";//stores the classification of complexity as string N,NN,NL
    var bigOList = [];//  will store the complexity at first level
    var hasError = false;
    var isLazy = false;
    var evalChars = [];//stores chars that describe the complexity in an array
    var levelOneIndex = 0;
